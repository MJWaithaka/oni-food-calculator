...
for(const file of fs.existsSync('wiki-cache')?fs.readdirSync('wiki-cache'):[]){
 const d=JSON.parse(fs.readFileSync(`wiki-cache/${file}`));const html=d.parse?.text?.['*']||'';
 for(const m of html.matchAll(/<img\b[^>]*>/g)){
  const alt=m[0].match(/\balt="([^"]*)"/),src=m[0].match(/\bsrc="([^"]+)"/);
  if(!alt||!src)continue;
  const name=alt[1].replace(/\.png$/,'').replace(/^Resource /,'').replace('Souffle Pancakes','Soufflé Pancakes').replace('Surf N Turf',"Surf'n'Turf");
  let url=src[1].replace('/images/thumb/','/images/').replace(/\.png\/[^?]+/,'.png');
  if(url.startsWith('/images/')&&/\.png(?:\?|$)/.test(url))images.set(name,new URL(url,'https://oxygennotincluded.wiki.gg').href);
 }
}
